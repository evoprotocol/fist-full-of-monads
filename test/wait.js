...
describe("waitForReceipt", () => {
  it("gets receipt from provider", async () => {
  ...
  it("gets receipt when the transaction's block was already mined", async () => {
    const receipt = { dummy: true };
...
describe("waitForContract", () => {
  ...
  it("fails if no code was found", async () => {
    const getCode = sinon.stub();
    getCode.returns("0x");
    const receipt = { contractAddress: "0x7967c4f4512195ba83ae8f08ca30f7b145be6cf8" };
    const provider = testutils.stubbedProvider(
    ...
    let threw = false;
    try {
      await utils.waitForContract("0xe8d3266f3c4f083ab8864b5e04aea7b087044e49", provider);
    } catch (error) {
      threw = true;
    }
